...
df_opt_rnd_95_new = readRDS(paste0('./df_opt_rnd_95_',loop_over,'_2.rds'))
df_opt_rnd_95_old = readRDS(paste0('./df_opt_rnd_95_',loop_over,'.rds'))
df_opt_rnd_95     = setdiff(df_opt_rnd_95_new, df_opt_rnd_95_old)

chunks = split_equal(1:dim(df_opt_rnd_95)[1], n1)
loop_over_param_inds = chunks[[n2]]

# == 60800
# params_df[param_names] = c(0.0325, 0.0403, 0.4607, 0.9685, 0.0325) ## OPTIMIZED FOR 60800_A - WORKS BEAUTIFULLY FOR _A!

for (i_opt in loop_over_param_inds){

  print(paste0('At i_opt', i_opt))
  params_df[param_names] = df_opt_rnd_95[i_opt,]  ## OPTIMIZED FOR 60800_C
  ...
  scenarios_df = expand.grid(
    ...
    randomize_tregs = c(0),
    macspec_on      = c(0),
    ros_level       = c(2), # 0 is control - max(ros_level) x max(add_ROS) = 2 x 0.5 = 1 (anyway capped at 1 so makes sense)
    pat_level       = c(10)
    # ros_level       = c(13:16), # 0 is control - max(ros_level) x max(add_ROS) = 2 x 0.5 = 1 (anyway capped at 1 so makes sense)
