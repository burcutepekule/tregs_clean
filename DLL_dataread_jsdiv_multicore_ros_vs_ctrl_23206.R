...
all_comparison_results = all_comparison_results %>% dplyr::filter(score_type=='epithelium' & control==0)

summary_df_param_set = all_comparison_results %>%
  group_by(param_set_id) %>%
  summarise(
    min_acf_regularity  = min(acf_regularity),
    max_acf_regularity  = max(acf_regularity),
    mean_acf_regularity = mean(acf_regularity),
    min_acf_peak_val  = min(acf_peak_val),
    max_acf_peak_val  = max(acf_peak_val),
