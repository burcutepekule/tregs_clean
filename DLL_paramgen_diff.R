...
param_bounds = list(
  th_ROS_microbe        = c(0, 0.49),  # th_ROS_microbe should be lower than th_ROS_epith_injury
  th_ROS_epith_injury   = c(0.51, 1), # th_ROS_microbe should be lower than th_ROS_epith_injury
  epith_recovery_chance = c(0.05, 0.05), #FIXED
  diffusion_speed_DAMPs = c(0.01, 0.12),
  diffusion_speed_PAMPs = c(0.01, 0.12),
  diffusion_speed_SAMPs = c(0.01, 0.12),
  diffusion_speed_ROS   = c(0.01, 0.12),
  add_DAMPs = c(0.05, 0.5),
  add_SAMPs = c(0.05, 0.5),
  add_PAMPs = c(0.05, 0.5),
  ros_decay   = c(0.05, 0.25),
  DAMPs_decay = c(0.05, 0.25),
  SAMPs_decay = c(0.05, 0.25),
  PAMPs_decay = c(0.05, 0.25),
  activation_threshold_danger = c(0.1, 2), # because we sum DAMPs and PAMPs now, it should double!
  activation_threshold_SAMPs  = c(0.1, 1),
  activity_engulf_M0_baseline = c(0, 0), #FIXED
  activity_engulf_M1_baseline = c(0.05, 0.50),
  activity_ROS_M1_baseline    = c(0.1, 1),
  rate_leak_commensal_injury   = 0.01*c(0.25, 0.25), #FIXED
  rate_leak_commensal_baseline = 0.01*c(0.05, 0.05), #FIXED
  treg_discrimination_efficiency = c(0, 1),
  ### =========== PARAMETERS EXCLUSIVE TO ABM MODEL ========== ###
  active_age_limit = c(1, 30), # discrete parameter, will be rounded
  cc_phagocyte     = c(1, 30), # discrete parameter, will be rounded
  recruitment_rate_danger = c(0, 0.25),  # Macrophage recruitment rate from borders (proportional to danger signal)
  ### =========== PARAMETERS EXCLUSIVE TO ABM MODEL ========== ###
  ### =========== PARAMETERS EXCLUSIVE TO DFF MODEL ========== ###
  diffusion_speed_microbe = c(0.01, 0.12), 
  decay_rate_microbe      = c(0.1, 0.75), 
  diffusion_speed_macro   = c(0.01, 0.12)
...
# Set parameters
set.seed(123)
n_samples = 1e4  # Total number of LHS samples to generate - make it 10 times higher for filtering later
...
# For ubelix
# lhs_samples$param_set_id = lhs_samples$param_set_id+nrow(lhs_samples)

# Reorder columns to put param_set_id first
lhs_samples = lhs_samples[c('param_set_id', param_names)]

saveRDS(lhs_samples, 'lhs_parameters_dff.rds')
# # Export
# output_file = "./lhs_parameters_diff.csv"
