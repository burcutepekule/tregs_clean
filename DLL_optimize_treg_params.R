...
cat("Treg parameters to optimize:", paste(names(treg_param_bounds), collapse=", "), "\n\n")
...
# ============================================================================
# SCENARIO DEFINITIONS (same as DLL_datagen_abm.R)
# ============================================================================

scenarios_df = expand.grid(
  control         = c(0),
  sterile         = c(0),
  allow_tregs     = c(1), # case 0 will be automatically generated below! 
  randomize_tregs = c(0),
  macspec_on      = c(0),
  ros_level       = c(0, 1),
  pat_level       = c(1, 2, 3)
...
cat("Optimizing for scenario", scenario_ind_use, "\n\n")

# ============================================================================
# OBJECTIVE FUNCTION
# ============================================================================

# This function runs simulations with given Treg parameters and computes
# a metric representing the benefit of Tregs (to be maximized)
run_simulation_with_treg_params = function(treg_params, base_params, scenario, verbose=FALSE) {
  
  # Extract base parameters
  param_set_use = base_params
  
  # Add Treg parameters to param_set_use
  param_set_use$rat_com_pat_threshold = treg_params[1]
  param_set_use$diffusion_speed_SAMPs = treg_params[2]
  param_set_use$add_SAMPs = treg_params[3]
  param_set_use$SAMPs_decay = treg_params[4]
  param_set_use$activation_threshold_SAMPs = treg_params[5]
  param_set_use$treg_discrimination_efficiency = treg_params[6]
  
  # Extract scenario parameters
  sterile         = scenario$sterile
  allow_tregs     = scenario$allow_tregs
  randomize_tregs = scenario$randomize_tregs
  macspec_on      = scenario$macspec_on
  control         = scenario$control
  ros_level       = scenario$ros_level
  pat_level       = scenario$pat_level
  ...
  for (reps_in in 0:(num_reps-1)){
    ...
    time_ss_e = unique(longitudinal_df$time_ss_e)
    time_ss_p = unique(longitudinal_df$time_ss_p)
    
    if(any(is.na(time_ss_e), is.na(time_ss_p))){
      scores_e_keep = c(scores_e_keep, NA)
      scores_p_keep = c(scores_p_keep, NA)
    }else{
      scores_e_keep = c(scores_e_keep, longitudinal_df$epithelial_score[time_ss_e:t_max])
      scores_p_keep = c(scores_p_keep, longitudinal_df$pathogen[time_ss_p:t_max])
    }
  }
  # ==========================================
  # Compute summary metrics at steady state
  
  # Average across replicates
  final_results = data.frame(
    pathogen = mean(scores_p_keep, na.rm = TRUE),
    epithelial_score = mean(scores_e_keep, na.rm = TRUE)
  )
  
  return(final_results)
}

# Objective function for optimization (returns negative value because optim minimizes)
objective_function = function(treg_params_vector, base_params, scenario_without_tregs, scenario_with_tregs) {
  
  # Ensure parameters are within bounds
  treg_params_vector = pmax(treg_params_vector, sapply(treg_param_bounds, function(x) x[1]))
  treg_params_vector = pmin(treg_params_vector, sapply(treg_param_bounds, function(x) x[2]))
  
  # Run simulation WITHOUT Tregs (baseline)
  results_no_tregs = run_simulation_with_treg_params(
    treg_params_vector, base_params, scenario_without_tregs
  )
  
  # Run simulation WITH Tregs
  results_with_tregs = run_simulation_with_treg_params(
    treg_params_vector, base_params, scenario_with_tregs
  )
  
  # Compute benefit score (higher is better)
  # We want Tregs to:
  # 1. Reduce pathogen burden (negative weight)
  # 3. Improve epithelial health (positive weight)
  # 4. Reduce M1 inflammation (negative weight)
  # 5. Increase M2 anti-inflammatory (positive weight)
  
  benefit_score =
    -1.0 * (results_with_tregs$pathogen - results_no_tregs$pathogen) +  # Lower pathogen is better
    +10.0 * (results_with_tregs$epithelial_score - results_no_tregs$epithelial_score)  # Higher score is better
  
  cat(sprintf("Benefit score: %.4f | Params: [%.3f, %.3f, %.3f, %.3f, %.3f, %.3f]\n",
              benefit_score,
              treg_params_vector[1], treg_params_vector[2], treg_params_vector[3],
              treg_params_vector[4], treg_params_vector[5], treg_params_vector[6]))
  
  # Return negative because optim minimizes
...
cat("Starting optimization for parameter set", param_set_id_use, "scenario", scenario_ind_use, "\n\n")

# Get base parameters
base_params = params_df %>% filter(param_set_id == param_set_id_use)

# Get scenario
scenario_row = scenarios_df[scenario_ind_use, ]

# Only optimize if this scenario has allow_tregs=1
if (scenario_row$allow_tregs == 1) {
  
  # Create scenario without Tregs for comparison
  scenario_without_tregs = scenario_row
  scenario_without_tregs$allow_tregs = 0
  
  scenario_with_tregs = scenario_row
  
  # Initial guess (midpoint of bounds)
  initial_params = sapply(treg_param_bounds, function(x) mean(x))
  
  # Parameter bounds as vectors
  lower_bounds = sapply(treg_param_bounds, function(x) x[1])
  upper_bounds = sapply(treg_param_bounds, function(x) x[2])
  
  cat("Initial parameters:", paste(round(initial_params, 3), collapse=", "), "\n")
  cat("Lower bounds:", paste(round(lower_bounds, 3), collapse=", "), "\n")
  cat("Upper bounds:", paste(round(upper_bounds, 3), collapse=", "), "\n\n")
  
  # Optimization using L-BFGS-B (box-constrained optimization)
  opt_result = optim(
    par = initial_params,
    fn = objective_function,
    base_params = base_params,
    scenario_without_tregs = scenario_without_tregs,
    scenario_with_tregs = scenario_with_tregs,
    method = "L-BFGS-B",
    lower = lower_bounds,
    upper = upper_bounds,
    control = list(maxit = 50, trace = 6)  # Limit iterations for computational feasibility
  )
  
  # Extract optimized parameters
  optimized_params = opt_result$par
  names(optimized_params) = names(treg_param_bounds)
  
  cat("\n\nOptimization complete!\n")
  cat("Optimized parameters:\n")
  print(data.frame(
    parameter = names(optimized_params),
  ...
  output_file = sprintf("%s/optimized_treg_params_set_%d_scenario_%d.rds",
                        output_dir, param_set_id_use, scenario_ind_use)
  saveRDS(results, output_file)
  cat("Results saved to:", output_file, "\n")
  
} else {
  cat("Scenario", scenario_ind_use, "does not use Tregs (allow_tregs=0). Skipping optimization.\n")
}
