...
param_tests = param_tests %>%
  ...
  arrange(p_worse_vs_neutral)

print("Parameters significantly different between regions:")
print(param_tests %>% filter(adj_p_worse_vs_neutral < 0.05))
...
p_values_for_plot = param_tests %>%
  ...
  dplyr::select(parameter, adj_p_worse_vs_neutral) %>%
  mutate(
    p_label_2 = case_when(
    ...
    p_text = sprintf("p_bvn = %.2e %s", adj_p_worse_vs_neutral, p_label_2)
  )

# Boxplots for top parameters
df_plot_params = df_analysis %>%
  dplyr::filter(region %in% c("Worse only", "Don't matter")) %>%
...
p_params = ggplot(df_plot_params, aes(x = region, y = value, fill = region)) +
  ...
  # ) +
  geom_signif(
    comparisons = list(c("Worse only", "Don't matter")),
    test = "t.test",
