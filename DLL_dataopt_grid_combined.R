...
# Create a named list of sequences
param_seqs = setNames(
  lapply(param_names, function(p) {
    seq(param_bounds[p, "lower"], param_bounds[p, "upper"], length.out = 5)
  }),
  param_names
)

# Expand the grid
params_opt = distinct(do.call(expand.grid, param_seqs))
dim(params_opt)
...
for (ind_opt in 1:dim(params_opt)[1]){
  scenarios_df = rbind(scenarios_df, expand.grid(
    ...
    ros_level       = seq(6,10,1), # MAX 10! 0 is control - max(ros_level) x max(add_ROS) = 2 x 0.5 = 1 (anyway capped at 1 so makes sense)
    pat_level       = c(10),
    overwrite       = c(0),
...
saveRDS(scenarios_df, './GRID_opt.rds')
dim(scenarios_df)[1]
...
dir_name_data = '/scratch/gpfs/CMETCALF/sim_opt_grid'
