...
# ros_vals        = c(5, 7, 10)# 0 is control - max(ros_level) x max(add_ROS) = 2 x 0.5 = 1 (anyway capped at 1 so makes sense)
# pat_vals        = c(10, 15, 20, 50)
ros_vals        = c(6:10)# 0 is control - max(ros_level) x max(add_ROS) = 2 x 0.5 = 1 (anyway capped at 1 so makes sense)
# pat_vals        = seq(3,3.9,0.1)
pat_vals        = seq(3.4, 3.9, 0.1)
...
# matching_rows = matching_rows %>% dplyr::filter(pat==20 & ros==5) # TO CHECK 
# matching_rows = matching_rows %>% dplyr::filter(pat==20 & ros==20) # TO CHECK 
