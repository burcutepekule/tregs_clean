...
df_comparisons = df_comparisons %>% dplyr::mutate(diff_tregs_on_minus_off = ifelse(injury_type=='pathogenic', 
                                                                                   mean_macspec_pathogen_e-mean_tregs_off_pathogen_e,
                                                                                   mean_macspec_sterile_e-mean_tregs_off_sterile_e))
...
df_comparisons = merge(df_comparisons, distinct(df_results[c('param_set_id',
                                                             'd_macspec_vs_tregs_off_pathogen_e',
                                                             'd_macspec_vs_tregs_off_sterile_e')]), by='param_set_id')
...
df_comparisons_plot_pathogenic = df_comparisons_plot_pathogenic %>% dplyr::mutate(tregs_better_cohens = ifelse(abs(d_macspec_vs_tregs_off_pathogen_e)>jsd_th,
                                                                                                               tregs_better, 0))

df_comparisons_plot_pathogenic = df_comparisons_plot_pathogenic %>% dplyr::select(-d_macspec_vs_tregs_off_sterile_e)
colnames(df_comparisons_plot_pathogenic)[which(colnames(df_comparisons_plot_pathogenic)=='d_macspec_vs_tregs_off_pathogen_e')]='cohens_d'
...
df_comparisons_plot_sterile = df_comparisons_plot_sterile %>% dplyr::mutate(tregs_better = ifelse(diff_tregs_on_minus_off > tol_in, 1,
                                                                                                  ifelse(diff_tregs_on_minus_off < -1*tol_in,-1,0)))
df_comparisons_plot_sterile = df_comparisons_plot_sterile %>% dplyr::mutate(tregs_better_cohens = ifelse(abs(d_macspec_vs_tregs_off_sterile_e)>jsd_th,
                                                                                                         tregs_better, 0))

df_comparisons_plot_sterile = df_comparisons_plot_sterile %>% dplyr::select(-d_macspec_vs_tregs_off_pathogen_e)
colnames(df_comparisons_plot_sterile)[which(colnames(df_comparisons_plot_sterile)=='d_macspec_vs_tregs_off_sterile_e')]='cohens_d'
...
ggsave(
  filename = paste0("./ABM_JSD_MACSPEC_vs_BASELINE.png"),
  plot = p,
  width = 9,
  height = 6,
  dpi = 300,
