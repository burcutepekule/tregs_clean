...
for (param_id in c(147, 172, 222, 269)){
  ...
  i_opt_vec = 1
  m2on_vec  = c(0)
  for (effidx in effidx_vec){
    for (m2on in m2on_vec){
      for(i_opt in i_opt_vec){
        print(c(effidx, m2on, i_opt))
  ...
  n_other    = length(effidx_vec)   # number of non-zero values
  blues_func = colorRampPalette(RColorBrewer::brewer.pal(n_other + 1, "Blues"))
  blues_cols = blues_func(n_other + 1)[-1]
  custom_colors = c("-1" = "red", setNames(blues_cols, as.character(effidx_vec)))
  
  p_opt = ggplot(col_avg_keep, aes(x = ros_level, y = avg_pct, color = factor(eff))) +
  ...
  ggsave(
    filename = paste0(path_img, "/param_",param_id,"_OPTS_eff_m2_OFF.png"),
    plot = p_opt,
    width = 16,
    height = 8,
    dpi = 300,
