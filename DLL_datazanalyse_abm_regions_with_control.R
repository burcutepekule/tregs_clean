...
df_comparisons = df_comparisons %>% dplyr::mutate(diff_tregs_on_minus_off = ifelse(injury_type=='pathogenic', 
                                                                                   mean_tregs_on_pathogen_e-mean_tregs_off_pathogen_e,
                                                                                   mean_tregs_on_sterile_e-mean_tregs_off_sterile_e))
...
df_comparisons = merge(df_comparisons, distinct(df_results[c('param_set_id',
                                                             ...
                                                             'd_tregs_on_vs_off_sterile_e',
                                                             'd_tregs_rnd_vs_nrnd_sterile_e')]), by='param_set_id')

#----- filter based on cohen's d since you are looking for differences!
df_comparisons_below= df_comparisons %>% dplyr::filter((injury_type=='pathogenic' & abs(d_tregs_on_vs_off_pathogen_e)<jsd_th)|(injury_type=='sterile' & abs(d_tregs_on_vs_off_sterile_e)<jsd_th))
...
df_comparisons_sense = df_comparisons %>% dplyr::mutate(makes_sense=ifelse(sign(diff_tregs_on_minus_off)!=sign(diff_notrnd_minus_rnd) 
                                                                           ...
                                                                           & (abs(d_tregs_rnd_vs_nrnd_pathogen_e)>=jsd_th | abs(d_tregs_rnd_vs_nrnd_sterile_e)>=jsd_th) ,0,1)) #either for sterile or pathogenic
...
df_comparisons_plot_pathogenic = df_comparisons_plot_pathogenic %>% dplyr::mutate(tregs_better_cohens = ifelse(abs(d_tregs_on_vs_off_pathogen_e)>jsd_th,
                                                                                                               tregs_better, 0))

df_comparisons_plot_pathogenic = df_comparisons_plot_pathogenic %>% dplyr::select(-d_tregs_on_vs_off_sterile_e)
colnames(df_comparisons_plot_pathogenic)[which(colnames(df_comparisons_plot_pathogenic)=='d_tregs_on_vs_off_pathogen_e')]='cohens_d'
...
df_comparisons_plot_sterile = df_comparisons_plot_sterile %>% dplyr::mutate(tregs_better = ifelse(diff_tregs_on_minus_off > tol_in, 1,
                                                                                                  ifelse(diff_tregs_on_minus_off < -1*tol_in,-1,0)))
df_comparisons_plot_sterile = df_comparisons_plot_sterile %>% dplyr::mutate(tregs_better_cohens = ifelse(abs(d_tregs_on_vs_off_sterile_e)>jsd_th,
                                                                                                         tregs_better, 0))

df_comparisons_plot_sterile = df_comparisons_plot_sterile %>% dplyr::select(-d_tregs_on_vs_off_pathogen_e)
colnames(df_comparisons_plot_sterile)[which(colnames(df_comparisons_plot_sterile)=='d_tregs_on_vs_off_sterile_e')]='cohens_d'
